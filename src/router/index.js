...
export default class Router {
  ...
  loadView(currentURL) {
    ...
    this.currentURLlist.forEach((item, index) => {
      ...
      this.url += "/" + item
      this.constrollerName = this.routes[this.url]
      // 404页面处理
      if (!this.constrollerName) {
        this.errorPage()
        return false
      }
      // 对于嵌套路由的处理
      if (this.oldURL && this.oldURL[index] == this.currentURLlist[index]) {
        ...
        this.controller(this.constrollerName,item)
      }
    });
    // 记录链接数组,后续处理子级组件
  ...
  controller(name,item) {
    name.render();           // 当前路由匹配的  controller
    this.navActive(item)
  }
  /**
   * 手动跳转路由
  ...
  navActive(item) {
    ...
    //   $(this.navList[i]).closest('li').removeClass('active')
    //   if ($(this.navList[i]).attr('href').slice(2) === item) {
    //     $(this.navList[i]).closest('li').addClass('active')
    //   }
    // }
    // this.navList.each( (index,value) => {
    //   if($(valur).attr('href').slice(2) == item) {
